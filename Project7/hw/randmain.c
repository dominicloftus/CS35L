  ...
  if (rdrand_supported ())
    {
      ...
      if(!dl_handle){
	fprintf(stderr,"Could not open randlibhw: %s\n", dlerror());
	exit(1);
      }
      rand64 = dlsym(dl_handle, "rand64");
      error = dlerror();
      if(error != NULL){
	fprintf(stderr,"Could not find rand64: %s\n", error);
	exit(1);
      }
    }
  else
    {
      ...
      if(!dl_handle){
	fprintf(stderr,"Could not open randlibso: %s\n", dlerror());
	exit(1);
      }
      rand64 = dlsym(dl_handle, "rand64");
      error = dlerror();
      if(error != NULL){
	fprintf(stderr,"Could not find rand64(sw): %s\n", error);
	exit(1);
      }
    }
